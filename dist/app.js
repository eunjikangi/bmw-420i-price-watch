...
function render(){
 ...
 const c=$('#content');
 if(tab==='all'||tab==='bps')c.innerHTML=table(rs)+`<p class="subnote">${tab==='bps'?'BMW BPS 표시를 확인한 차량만 표시합니다. 인증 표시가 개별 보증 잔여를 뜻하지는 않습니다. ':''}조회 실패 차량은 ‘이전 정상 자료 보존’으로 표시합니다. ‘광고 게시 중’은 상세페이지가 열렸다는 뜻입니다. 금융상품 가입 없는 구매가격·총인수비용·실재고는 별도 확인이 필요합니다. 가로로 스크롤하면 모든 비교 항목을 볼 수 있습니다.</p>`;
