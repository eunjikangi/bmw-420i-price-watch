...
async function load(){
 let local;try{const r=await fetch('./data.json',{cache:'no-store'});if(!r.ok)throw Error('자료 요청 실패');local=await r.json();data=local;setup();}catch{throw Error('현재 데이터를 불러올 수 없습니다. 잠시 후 새로고침해 주세요.');}
