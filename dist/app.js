...
function setup(){
 $('#filters').innerHTML=select('color','외장 색상')+range('price','일반 광고가격 (만원)','최소','제한 없음')+range('year','모델연도','최소','최대')+range('km','주행거리 (km)','최소','제한 없음')+select('region','판매 지역')+select('trim','트림')+select('facelift','부분변경')+select('sale','판매 방식')+select('cert','인증 종류')+select('history','이력 확인 상태');
 ...
 $('#content').addEventListener('click',e=>{const b=e.target.closest('[data-record]');if(b)openRecord(b.dataset.record);});
...
function summary(){
 ...
 const wh=fresh.filter(g=>g.offers.some(isWhite));const latest=data.runs?.at(-1)||{};
 const status=data.sources.reduce((a,s)=>(a[s.status]=(a[s.status]||0)+1,a),{});
 $('#summary').innerHTML=`<span>중복 확인 후 <strong>${fresh.length}</strong>대</span><span>흰색 <strong>${wh.length}</strong>대</span><span>이번 수집 신규 <strong>${latest.newCount||0}</strong> · 가격 변경 <strong>${latest.priceChangeCount||0}</strong></span><span>출처 <strong>${status['조회 성공']||0}</strong> 성공 · <strong>${status['일부만 조회']||0}</strong> 일부 · <strong>${data.sources.length-(status['조회 성공']||0)-(status['일부만 조회']||0)-(status['정상 검색 결과 대상 매물 없음']||0)}</strong> 제한/오류</span><span class="time">마지막 수집 ${fulltime(data.updatedAt)} KST</span>`;
}
...
function match(r){
 ...
 if(f.cert&&(f.cert==='미확인'?!!r.certification:f.cert==='기타'?(!r.certification||['BMW BPS','KB진단'].includes(r.certification)):r.certification!==f.cert))return false;
...
function rows(){
 let result=data.vehicles.map(g=>{let os=g.offers.filter(r=>match(r)&&r.status==='광고 게시 중'&&!r.stale);if(tab==='bps')os=os.filter(r=>r.certification==='BMW BPS');if(!os.length)return null;os.sort((a,b)=>Number(a.stale)-Number(b.stale)||new Date(b.lastSuccess)-new Date(a.lastSuccess));return {...os[0],group:g};}).filter(Boolean);
 const nullable=(a,b,dir=1)=>a==null?(b==null?0:1):b==null?-1:(a-b)*dir;
 result.sort((a,b)=>sort==='price-up'||sort==='price-down'?nullable(finance(a)?null:a.price,finance(b)?null:b.price,sort==='price-up'?1:-1):sort==='year'?nullable(a.modelYear,b.modelYear,-1):sort==='mileage'?nullable(a.mileage,b.mileage):Number(isWhite(b))-Number(isWhite(a))||Number(a.stale)-Number(b.stale)||new Date(b.lastSuccess)-new Date(a.lastSuccess));
 return result;
}
...
function row(r){return `<tr><td class="model-cell"><button class="row-open" data-record="${esc(r.id)}">420i 컨버터블<br>${val(r.trim)}</button><span class="cell-note">${esc(r.source)} · ${esc(r.listingId)}</span>${r.group?.offers.length>1?`<span class="cell-note">동일 차량 ${r.group.offers.length}개 출처</span>`:''}${r.group?.duplicateStatus==='중복 추정'?'<span class="badge warn">중복 추정</span>':''}</td><td class="num price-cell">${priceCell(r)}</td><td>${val(r.modelYear)}<span class="cell-note">모델연도</span></td><td>${val(r.registration)}</td><td class="num">${n(r.mileage)}<span class="cell-note">km</span></td><td>${isWhite(r)?'<span class="badge">흰색</span>':''}<span class="cell-note">${val(r.color)}</span></td><td>${val(r.region)}</td><td>${val(r.facelift)}</td><td class="condition-cell">${esc(r.saleMethod)}<span class="cell-note">금융 가입 없는 현금가 ${r.cashConfirmed?'확인':'미확인'}</span>${finance(r)?'<span class="cell-note warn">총비용 확인 필요</span>':''}</td><td class="condition-cell">${val(r.certification)}<span class="cell-note">${esc(r.inspection)}</span>${r.repair?'<span class="cell-note error">손상·수리 조건 확인</span>':''}<span class="cell-note">보험 ${r.insuranceCount==null?'미확인':r.insuranceCount+'건 · 상세 확인 필요'}</span></td><td><span class="badge ${r.stale?'warn':''}">${esc(r.status)}</span><span class="cell-note time-display">정상 ${time(r.lastSuccess)}</span><span class="cell-note time-display">조회 ${time(r.checkedAt)}</span>${r.stale?'<span class="cell-note error">이전 정상 자료 보존</span>':''}</td><td>${link(r.url,'원문')}<span class="cell-note"><button class="row-open" data-record="${esc(r.id)}">상세·이력</button></span></td></tr>`;}
function table(rs){return rs.length?`<div class="table-wrap"><table><thead><tr><th>차량 / 출처</th><th class="num">광고금액 (만원)</th><th>모델연도</th><th>최초등록</th><th class="num">주행거리</th><th>외장색</th><th>지역</th><th>부분변경</th><th>판매 조건</th><th>인증 / 이력</th><th>상태 / 확인시각</th><th>확인</th></tr></thead><tbody>${rs.map(row).join('')}</tbody></table></div>`:`<div class="empty">조건에 맞는 확인 자료가 없습니다. 수집 실패나 미확인 자료는 매물 0건의 근거가 아닙니다.</div>`;}
function render(){
 summary();document.querySelectorAll('[data-tab]').forEach(b=>{b.classList.toggle('active',b.dataset.tab===tab);b.setAttribute('aria-current',b.dataset.tab===tab?'page':'false');});
 const showFilters=['all','recommend','bps'].includes(tab);$('#filters').hidden=!showFilters;$('#sort-label').hidden=!showFilters;$('#reset').hidden=!showFilters;
 let rs=rows();$('#result-count').innerHTML=showFilters?`조건에 맞는 차량 <strong>${rs.length}</strong>대 · 금액은 광고값, 추가비용 별도`:'공개 자료와 관측 이력';
 const c=$('#content');
 if(tab==='all'||tab==='bps')c.innerHTML=table(rs)+`<p class="subnote">${tab==='bps'?'BMW BPS 표시를 확인한 차량만 표시합니다. 인증 표시가 개별 보증 잔여를 뜻하지는 않습니다. ':''}‘광고 게시 중’은 상세페이지가 열렸다는 뜻입니다. 금융상품 가입 없는 구매가격·총인수비용·실재고는 별도 확인이 필요합니다. 가로로 스크롤하면 모든 비교 항목을 볼 수 있습니다.</p>`;
...
function newCars(){
 ...
 out+=entries.length?`<div class="table-wrap"><table class="small-table"><thead><tr><th>차량 / 트림</th><th class="num">가격 (만원)</th><th>가격 종류</th><th>기준월</th><th>확인 시각</th><th>재고 / 조건</th><th>출처</th></tr></thead><tbody>${entries.map(p=>`<tr><td>${esc(p.model)}<span class="cell-note">${esc(p.trim)}</span></td><td class="num"><strong>${won(p.price)}</strong></td><td>${esc(p.priceKind)}</td><td>${esc(p.effectiveMonth)}${p.effectiveMonth!==month?'<span class="cell-note warn">이전 기준월 · 이번 달 가격 아님</span>':''}</td><td>${time(p.checkedAt)}</td><td>${esc(p.stock)}<span class="cell-note">${esc(p.conditions)}</span></td><td>${link(p.url,p.source)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">당월 신차 가격을 확인하지 못했습니다. 오래된 프로모션을 이번 달 가격으로 대신하지 않습니다.</div>';
 out+='<h2>함께 비교할 중고차</h2><p class="quiet">트림·모델연도·장비·판매조건이 다르므로 동일 사양의 할인율로 계산하지 않습니다.</p>'+table(data.vehicles.map(g=>({...g.offers[0],group:g})));
 out+='<h3>아직 확인되지 않은 가격</h3><div class="table-wrap"><table><thead><tr><th>가격 구분</th><th>확인 상태</th></tr></thead><tbody><tr><td>이번 달 광고 할인</td><td>미확인</td></tr><tr><td>금융상품 조건부 신차 가격</td><td>미확인</td></tr><tr><td>출고 가능한 흰색 재고 견적</td><td>미확인</td></tr></tbody></table></div>';return out;
}
...
function registerTools(){const ctx=document.modelContext;if(!ctx?.registerTool)return;const ac=new AbortController();window.addEventListener('pagehide',()=>ac.abort(),{once:true});
 Promise.resolve(ctx.registerTool({name:'filter_420i_listings',description:'420i 비교 표의 색상 필터와 가격 정렬을 변경하고 보이는 매물 요약을 반환합니다.',inputSchema:{type:'object',properties:{color:{type:'string',enum:['all','white','other']},sort:{type:'string',enum:['white','price-up','price-down','year','mileage']}},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},execute(input){if(!input||typeof input!=='object'||Object.keys(input).some(k=>!['color','sort'].includes(k))||input.color&&!['all','white','other'].includes(input.color)||input.sort&&!['white','price-up','price-down','year','mileage'].includes(input.sort))throw Error('유효하지 않은 필터');tab='all';if(input.color){filters.color={all:'',white:'흰색 계열',other:'흰색 외'}[input.color];$('[data-filter="color"]').value=filters.color;}if(input.sort){sort=input.sort;$('#sort').value=sort;}render();return rows().map(r=>({id:r.id,price:r.price,priceKind:r.priceKind,year:r.modelYear,mileage:r.mileage,source:r.source,url:r.url}));}},{signal:ac.signal})).catch(()=>{});
}
async function load(){
 let local;try{const r=await fetch('./data.json',{cache:'no-store'});if(!r.ok)throw Error('자료 요청 실패');local=await r.json();data=local;}catch{throw Error('현재 데이터를 불러올 수 없습니다. 잠시 후 새로고침해 주세요.');}
