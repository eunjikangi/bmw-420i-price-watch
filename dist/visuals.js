...
function evidenceBadge(r){const k=historySignal(r),h=evidenceKinds[k];return '<span class="evidence '+k+'" title="'+esc(h.note)+'"><i aria-hidden="true"></i>'+h.label+((k==='insurance')?' · '+r.insuranceCount+'건':'')+'</span>';}
...
function mobileGraphPeek(r){return r?'<div class="peek-info"><strong>'+won(r.price)+'<small>만원</small></strong><span>'+val(r.modelYear)+'년형 · '+n(r.mileage)+'km</span></div><button class="primary-button" data-record="'+esc(r.id)+'">상세</button><button class="icon-close" data-clear-selection aria-label="차량 선택 해제">×</button>':'';}
function selectGraphRecord(id){
 ...
 const select=$('#graph-record-select');if(select)select.value=graphSelected||'';const peek=$('#graph-quick-peek');if(peek){peek.hidden=!r;peek.innerHTML=mobileGraphPeek(r);}
